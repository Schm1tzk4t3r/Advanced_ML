...
solely for backward compatibility with the existing Streamlit demo
(`pages/2_Dashboard.py`). It will be retired once Person 4 wires the demo
to `data/processed/vinhaguard_dataset.parquet`.  
**Do not use `douro_climate.parquet` for model training or evaluation.**
