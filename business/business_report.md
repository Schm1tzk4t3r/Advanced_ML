# VinhaGuard AI — Business and Insurance Report

> **Focus:** Problem framing, market logic, product concept, revenue model, unit economics, fairness, basis risk, go-to-market, and safety.
...
*Note: these probabilities reflect the prototype model (`model/predict.py`), grounded in the real dataset statistics from `data/processed/vinhaguard_dataset.parquet`. The trained Logistic Regression / Random Forest backend is implemented and provides the ML backend for risk probability estimation.*
...
**Current prototype status:** The risk probability outputs come from the trained Random Forest classifier (ROC-AUC 0.970 before calibration and 0.977 after calibration on a chronological 2020–2024 holdout), blended 70/30 with historical trigger rates. The full pipeline — data, feature engineering, TerraClimate water-balance enrichment, ML model, premium formula, and Streamlit app — is implemented and deployed.
...
| **Risk model** | Climate stress classification; trigger probability estimation; premium input generation | ✅ Trained Logistic Regression / Random Forest backend |
...
> These figures are **illustrative MVP assumptions**, not actuarial or investment-grade estimates. The risk probabilities are generated by the trained ML backend; full actuarial calibration remains a pre-commercialisation step.
...
The table below shows backend outputs from `predict_risk_and_premium()` — not illustrative assumptions. Premium figures use the trained Random Forest model blended 70/30 with the actual historical trigger rates shown above.
...
| ~8–12% trigger probability | Illustrative calibration | Narrower subset of stress years; exact value to be set through actuarial review |
...
| Risk probability outputs | ✅ Live — calibrated Random Forest ROC-AUC 0.977, blended 70/30 with historical trigger rates |
...
The business case is supported by the implemented data pipeline, ML model, pricing backend, and Streamlit demo in this repository.
