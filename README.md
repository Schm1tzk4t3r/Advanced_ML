...
```
locations      →  fetch_weather  →  build_features  →  fetch_terraclimate  →  make_dataset
```
...
| 4 | `python -m src.data.fetch_terraclimate` | `data/processed/terraclimate_features.parquet` `.csv` | Adds TerraClimate water-balance features: VPD, climate water deficit, soil moisture, precipitation |
| 5 | `python -m src.data.make_dataset` | `data/processed/vinhaguard_dataset.parquet` `.csv` | Adds `climate_stress_year` binary label and merges TerraClimate features; final ML-ready dataset |

**Runtime:** ~25–40 minutes end-to-end on first run, dominated by Step 2 and Step 4 network calls. Open-Meteo's free tier allows approximately 3–4 requests per minute; the weather fetch script sleeps 20 s between requests to stay within rate limits. TerraClimate extraction is cached by variable-year under `data/raw/terraclimate/`, so reruns are much faster. Step 3 and Step 5 each complete in under 10 seconds.
...
| Artifact | For | Notes |
|---|---|---|
| `data/processed/vinhaguard_dataset.parquet` | Person 3 (ML training), Person 4 (Streamlit demo) | Canonical dataset — 960 rows × 29 columns (32 locations × 30 years). Target column: `climate_stress_year` (39.8% positive class). |
| `data/processed/terraclimate_features.parquet` | Person 3 (ML training) | Water-balance enrichment — 960 rows × 10 columns, including VPD, climate water deficit, soil moisture, and precipitation summaries. |
| `model/artifacts/scored_history.csv` | Person 3 (ML backend), Person 4 (dashboard) | Rich scored history used by the current dashboard — 960 rows x 33 columns, including trigger indicators and model stress probabilities. |
